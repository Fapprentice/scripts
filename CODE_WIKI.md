...
> 当前架构说明，核对日期：2026-08-31。产品行为契约见 `PRODUCT_FLOW.md`，领域词汇见 `CONTEXT.md`。技能地图决策见 `docs/adr/0001-skill-map-as-plan-source.md`。
...
| `state_store.py` | SQLite 事务存储（schema v3：技能先修含 kind/rationale）、旧 JSON 迁移、内容寻址附件、完整性检查、滚动备份与恢复 |
...
| `learning.py` | 技能地图（计划真源）、FSRS 调度、掌握证据结算、诊断任务与执行材料 |
...
| `applog.py` | 结构化日志 |
| `evaluation.py` | AI 评测（含技能地图门禁）、语义裁判适配、发布门禁、校准与隐私安全抽样 |
...
- **记录**：复盘摘要、技能地图、最近 12 个月的月度热力图，以及归档/事件/退出记录弹窗
...
2. 学习型目标先绑定版本化技能包并检查图覆盖；无 pack 时才生成稳定能力维度。
3. 今日学习任务只从已解锁、未掌握或到期节点生成，并带上该节点的掌握证据。
4. 用户从首页开始、暂停和继续任务；计时累计保存。
5. 材料型任务在弹出面板内作答，外部成果通过文件上传。
6. 验收优先执行确定性规则，必要时再调用 AI；任务通过不等于技能已掌握。
7. 结果更新积分、FSRS、技能地图节点状态、反馈模型和下一轮容量。陪伴仍只跟任务验收。
